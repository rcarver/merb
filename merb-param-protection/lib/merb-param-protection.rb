            ...
            unless self.send(params_method).nil?
              ...
              args.keys.each do |key|
                ...
                if self.send(params_method).include?(key)
                  case method
                  when :accessible_params_args
                    raise "Cannot make accessible a controller (#{self}) that is already protected"
                  when :protected_params_args
                    raise "Cannot protect controller (#{self}) that is already accessible"
                  end
                end
              end
            end
